...
bool HomogeneousMedium::sampleDistance(PathSampleGenerator &sampler, const Ray &ray,
        MediumState &state, MediumSample &sample, float, GuideInfoConstPtr, bool) const
{
    ...
    if (_absorptionOnly) {
        ...
        // sample.continuedWeight = sigma_s * Tr_{cnt} / AVG_PDF(sigma_t * _transmittance->mediumPdf(continuedTau, state.firstScatter));
