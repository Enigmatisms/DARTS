...
class Microfacet
{
    enum DistributionEnum
    {
        ...
        Phong,
        GGX,
        PBRT_GGX,               // adopted from PBRT-v3 TrowbridgeReitzDistribution
    };
...
public:
    ...
    static float D(DistributionEnum dist, float alpha, const Vec3f &m)
    {
        ...
        default: {          // actually Tungsten matches pbrt-v3 (when alpha_x = alpha_y)
            float alphaSq = alpha*alpha;
            float cosThetaSq = m.z()*m.z();
            float tanThetaSq = max(1.0f - cosThetaSq, 0.0f)/cosThetaSq;
            float cosThetaQu = cosThetaSq*cosThetaSq;
    ...
    static float G1(DistributionEnum dist, float alpha, const Vec3f &v, const Vec3f &m)
    {
        ...
        } case PBRT_GGX: {              // hits differently
            float alphaSq = alpha*alpha;
            float cosThetaSq = v.z()*v.z();
            float tanThetaSq = max(1.0f - cosThetaSq, 0.0f)/cosThetaSq;
            return 0.5f * (-1.0f + std::sqrt(1.0f + alphaSq*tanThetaSq));
        }
        }
    ...
    {
        if (dist == PBRT_GGX) {         // difference is so minor
            // printf("GGX: %f, PBRT-GGX: %f, diff = %f, same = %d\n", value, result, std::abs(value - result), int(value == result));
            return 1.f / (1.f + G1(dist, alpha, i, m) + G1(dist, alpha, o, m));
        }
    ...
    static inline Vec2f invert(Distribution dist, float alpha, const Vec3d &m, float mu)
    {
        ...
        case Beckmann: {
            ...
            break;
        } default:      // PBRT_GGX does not support invert (since I don't know what `invert` means)
            xi.x() = 0.0;
        }
