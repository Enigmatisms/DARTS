...
: _distribution("ggx"),
  ...
  _ior(1.5),       // note that PBRT-v3 got the wrong parameter (etaI as 1.5 and etaT as 1)
  _enableT(true)
{
...
bool RoughDielectricBsdf::sampleBase(SurfaceScatterEvent &event, bool sampleR, bool sampleT,
        float roughness, float ior, Microfacet::Distribution distribution, bool no_transmit)
{
    ...
    float sampleAlpha = Microfacet::roughnessToAlpha(distribution, no_transmit ? roughness : sampleRoughness);
    ...
    float F = Fresnel::dielectricReflectance(1.f/ior, wiDotM, cosThetaT);
    float etaM = 1.0f/ior;
    if (wiDotM < 0.0f) {
        if (no_transmit) return false;
        etaM = ior;
    }
    ...
    // Why it seems that... it's kind of different from pbrt
    ...
    // This looks quite different from PBRT-v3's
    // actually, event.weight * event.pdf is cosine attenuated BRDF value (woDotN will be cancelled out)
        ...
        float roughness, float ior, Microfacet::Distribution distribution)
{
    ...
    float wiDotM = event.wi.dot(m);
    float woDotM = event.wo.dot(m);
    float F = Fresnel::dielectricReflectance(1.f/ior, wiDotM);
    float G = Microfacet::G(distribution, alpha, event.wi, event.wo, m);
    float D = Microfacet::D(distribution, alpha, m);
...
bool RoughDielectricBsdf::invertBase(WritablePathSampleGenerator &sampler, const SurfaceScatterEvent &event,
        bool sampleR, bool sampleT, float roughness, float ior, Microfacet::Distribution distribution, bool no_transmit)
{
    float wiDotN = event.wi.z();
    float woDotN = event.wo.z();
