...
protected:
    ...
    std::unique_ptr<TransientBuffer3f> _transientBuffer;
...
public:
    ...
    */
    ...
    TransientBuffer3f *transientBuffer()
    {
        return _transientBuffer.get();
    }

    void tryAddTransient(Vec3f* transients, Vec2u pixel) {
        if (_transientBuffer)
            _transientBuffer->addTransientSample(transients, pixel);
    }
    ...
    const TransientBuffer3f *transientBuffer() const
    {
        return _transientBuffer.get();
    }
