    ...
    bool renderScene()
    {
        ...
        try {
            ...
            while (!integrator.done()) {
                ...
                int current_spp = integrator.currentSpp();
                double elapsed_time = timer.elapsed();
                std::string time_out_str = "infinity";
                if (_timeout > 0.0) {
                    time_out_str = StringUtils::durationToString(_timeout);
                }
                auto elapsed   = StringUtils::durationToString(elapsed_time);
                auto remaining = StringUtils::durationToString(elapsed_time * double(maxSpp) / double(current_spp) - elapsed_time); 
                writeLogLine(tfm::format("Completed %d/%d spp. (%s | %s | timeout: %s)", integrator.currentSpp(), maxSpp, elapsed, remaining, time_out_str));
                timer.stop();
